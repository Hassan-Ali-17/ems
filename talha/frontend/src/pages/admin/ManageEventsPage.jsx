...
export default function ManageEventsPage() {
  ...
  return (
    <section className="container py-5">
      <PageHeader eyebrow="Admin" title="Manage events" description="Review, edit, and delete events from the central admin panel." action={<Link to="/organizer/events/new" className="btn btn-info fw-semibold">Create event</Link>} />
      {loading ? (
        <Loading />
      ) : (
        <div className="card border-0 shadow-soft">
          <div className="table-responsive">
            <table className="table align-middle mb-0">
              <thead><tr><th>Title</th><th>Date</th><th>Status</th><th>Seats</th><th className="text-end">Actions</th></tr></thead>
              <tbody>
                {events.map((event) => (
                  <tr key={event._id}>
                    <td>{event.title}</td>
